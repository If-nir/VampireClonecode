...
public class Slime02 : MonoBehaviour
{
    Transform target;
    public GameObject Slime03;
    float time;
    float moveSpeed;
    public float Cur_Hp;
    ...
    void Update()
    {
        ...
        if (time > 4.0f)
        {
            Instantiate(Slime03, new Vector3(transform.position.x + 0.1f, transform.position.y, 0), Quaternion.identity);
            Instantiate(Slime03, this.transform.position, Quaternion.identity);
